...
describe('SteamOpenIDService', () => {
    ...
    describe('getRedirectUrl', () => {
        ...
        beforeAll(() => {
            openIDAuthenticateSpy = jest.spyOn(service['relyingParty'], 'authenticate');
        });

        it('should return a valid URL', async () => {
            openIDAuthenticateSpy.mockImplementationOnce((_identifier, _immediate, callback) =>
                callback(undefined, 'https://steamcommunity.com/openid/login')
            );
            expect(await service.getRedirectUrl()).toBe('https://steamcommunity.com/openid/login');
        });

        it('should throw a ServiceUnavailableException is a Steam OpenID referral URL cannot be generated', async () => {
            openIDAuthenticateSpy.mockImplementationOnce((_identifier, _immediate, callback) =>
                callback(new Error('its chewsday init'), 'https://steamcommunity.com/openid/login')
            );
            await expect(service.getRedirectUrl()).rejects.toThrow(ServiceUnavailableException);
        });
    });

    describe('authenticate', () => {
        let verifyAssertionSpy: jest.SpyInstance, getSteamUserSummaryDataSpy: jest.SpyInstance;

        beforeAll(() => {
            // Using bracket notation to access a private property here. JS is weird!
            verifyAssertionSpy = jest.spyOn(service['relyingParty'], 'verifyAssertion');
            getSteamUserSummaryDataSpy = jest.spyOn(steamService, 'getSteamUserSummaryData');
        });
