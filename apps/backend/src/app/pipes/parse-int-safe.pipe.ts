 ...
 * none of our tables should ever exceed `MAX_SAFE_INTEGER` (`2^53 - 1`)
 * entries. However Prisma will (sensibly) error if a param has a value above
 * `MAX_SAFE_INTEGER`, so rather than handle that in service logic, we can
 * simply `400` immediately for any such request. **/
@Injectable()
