...
export class SteamGameGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const request: RawBodyRequest<FastifyRequest> = context
      .switchToHttp()
      .getRequest();

    if (typeof request.headers['id'] !== 'string')
      throw new BadRequestException(
        'Invalid SteamID. Should be a Steam ID 64 (decimal).'
      );
    ...
    if (!steamID)
      throw new BadRequestException(
        'Missing SteamID. Should be a Steam ID 64 (decimal).'
      );
    ...
    if (this.configService.getOrThrow('steam.useSteamTicketLibrary')) {
  ...
  private async verifyUserTicketOnlineAPI(
    ...
    const steamID = await this.steamService.tryAuthenticateUserTicketOnline(
      userTicket,
      appID
    );

    if (steamIDToVerify !== steamID)
      throw new UnauthorizedException('Invalid user ticket');
  }

  private verifyUserTicketLocalLibrary(
