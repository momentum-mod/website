...
export class AdminService {
  ...
  async deleteLeaderboardRun(
    ...
    const run = await this.db.leaderboardRun.findUnique({
      where,
      // These clog up the adminactivity UI and probably not very useful.
      // Can be read from replay header if needed.
      omit: { splits: true },
  ...
  async deleteUserRuns(
    ...
    // Copy all replay files to the deleted-runs location with bounded
    // concurrency, then bulk-delete all sources in one request.
    const copies = runs.map((run) => ({
      fromKey: runPath(run.replayHash),
      toKey: deletedRunPath(
    ...
    await Promise.all([
      this.fileStore
        .copyFiles(copies)
        .then(() =>
          this.fileStore.deleteFiles(copies.map(({ fromKey }) => fromKey))
        ),
      this.adminActivityService.create(
        adminID,
  ...
  private async deleteRunFiles(
    run: Omit<LeaderboardRun, 'splits'>
  ): Promise<void> {
