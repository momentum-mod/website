...
describe('LocalUserService', () => {
  ...
  beforeEach(waitForAsync(() => {
    ...
    localUserService = new LocalUserService(
      new AuthService(spy, <any>httpClientSpy, <any>routerSpy),
      spy,
      <any>httpClientSpy
    );
  }));
  ...
  describe('Unit Tests', () => {
    ...
    it('#updateUser() should update the users profile and return the updated version', () => {
      ...
      localUserService
        .updateUser(<any>{ id: '1', profile: { id: '1', bio: 'testNewBio' } })
        .subscribe(
          (value) => expect(value).toEqual(expectedUser, 'expected user'),
          fail
        );
    ...
    it('#getMapLibrary() should return the maps in the specified users library', () => {
      ...
      localUserService
        .getMapLibrary()
        .subscribe(
          (value) => expect(value).toEqual(expectedMapLibrary, 'expected maps'),
          fail
        );
      expect(httpClientSpy.get.calls.count()).toBe(1, 'one call');
    });
    ...
    it('#addMapToLibrary() adds map to the local users library', () => {
      ...
      localUserService
        .addMapToLibrary(expectedMap.id)
        .subscribe(
    ...
    it('#removeMapFromLibrary() should remove the specified map from the users library', () => {
      ...
      localUserService
        .removeMapFromLibrary(expectedMap.id)
        .subscribe(
          (value) => expect(value).toEqual(expectedMap, 'expected map'),
          fail
        );
    ...
    it('#checkFollowStatus() should return a json obj with two booleans to represent follow status ', () => {
      ...
      localUserService
        .checkFollowStatus(expectedUser)
        .subscribe(
          (value) =>
            expect(value).toEqual(
              expectedFollowStatus,
              'expected follow status'
            ),
          fail
        );
      expect(httpClientSpy.get.calls.count()).toBe(1, 'one call');
    ...
    it('#updateFollowStatus() should update the following status on the users profile ', () => {
      httpClientSpy.patch.and.returnValue(of(expectedUser));
      localUserService
        .updateFollowStatus(expectedUser, 1)
        .subscribe(
          (value) => expect(value).toEqual(expectedUser, 'expected user'),
          fail
        );
    ...
    it('#updateMapNotify() should update map notification status or create new if no existing notifcations', () => {
      ...
      localUserService
        .updateMapNotify(expectedMap.id, 1)
        .subscribe(
          (value) =>
            expect(value).toEqual(expectedMapNotify, 'expected map notify'),
          fail
    ...
    it('#disableMapNotify() should remove the user from map notifcations list', () => {
      ...
      localUserService
        .disableMapNotify(expectedMap.id)
        .subscribe(
          (value) =>
            expect(value).toEqual(expectedMapNotify, 'expected map notify'),
          fail
