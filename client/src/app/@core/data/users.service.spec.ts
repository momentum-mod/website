  ...
  describe('Unit Tests', () => {
    it('#getUsers() should return expected users', () => {
      ...
      usersService
        .getUsers()
        .subscribe(
          (value) => expect(value).toEqual(expectedUsers, 'expected users'),
          fail
        );
    ...
    it('#getFollowersOfUser() should return followers of the user', () => {
      ...
      usersService
        .getFollowersOfUser(expectedUser)
        .subscribe(
          (value) => expect(value).toEqual(expectedFollowers, 'expected user'),
          fail
        );
      expect(httpClientSpy.get.calls.count()).toBe(1, 'one call');
    });
    ...
    it('#getUserFollows() should return the users the local user is following', () => {
      ...
      usersService
        .getUserFollows(expectedUser)
        .subscribe(
          (value) => expect(value).toEqual(expectedFollowed, 'expected user'),
          fail
        );
      expect(httpClientSpy.get.calls.count()).toBe(1, 'one call');
    });
