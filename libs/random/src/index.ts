export const Random = {
  ...
  weightedBool: (probabilityOfTrue: number): boolean =>
  ...
  enumKey: <T extends object>(enum_: T): keyof T =>
    Random.element(
      Object.keys(enum_).filter((key) => !(Math.abs(Number.parseInt(key)) + 1))
    ) as keyof T,

  enumValue: <T extends object>(enum_: T): T[keyof T] =>
    enum_[Random.enumKey(enum_)],
  ...
  pastDateInYears: (years = 1): Date =>
    Random.date(Date.now() - years * 31_536_000, Date.now()),

  pastDateSince: (refDate: Date | number): Date =>
    Random.date(typeof refDate != 'number' ? +refDate : refDate, Date.now()),

  createdUpdatedDates: (
    startDateYearsBack?: number
